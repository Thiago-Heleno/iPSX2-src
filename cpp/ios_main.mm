...
- (void)scene:(UIScene *)scene willConnectToSession:(UISceneSession *)session options:(UISceneConnectionOptions *)connectionOptions {
    ...
    if (!s_settings_interface) {
        ...
        if (!static_cast<INISettingsInterface*>(s_settings_interface)->Load()) {
            ...
            // Clamping — research-validated settings for R&C and PS2 accuracy.
            // VU Extra prevents NaN/Inf (invisible player, Spiky Polygon Syndrome).
            // EE Full is required for correct bounding box and collision math.
            // 0=None  1=Normal  2=Extra  3=Full(EE) / Extra+Sign(VU)
            s_settings_interface->SetIntValue("EmuCore/CPU/Recompiler", "VU0ClampMode", 2); // Extra
            s_settings_interface->SetIntValue("EmuCore/CPU/Recompiler", "VU1ClampMode", 2); // Extra
            s_settings_interface->SetIntValue("EmuCore/CPU/Recompiler", "FPU.ClampMode", 3); // Full

            // Gamefixes
            // XgKickHack: syncs XGKICK → prevents stale PATH1 geometry / player flicker.
            // VuAddSubHack: ADDi/SUBi precision for Insomniac engine.
            // EETimingHack: prevents black-screen hangs on cutscene transitions (Maktar Nebula).
            s_settings_interface->SetBoolValue("EmuCore/Gamefixes", "XgKickHack", true);
            s_settings_interface->SetBoolValue("EmuCore/Gamefixes", "VuAddSubHack", true);
            s_settings_interface->SetBoolValue("EmuCore/Gamefixes", "EETimingHack", true);

            // GS — hardware rendering quality and post-processing alignment.
            // HWMipmap: enables accurate LOD matching the PS2 GS; fixes black/missing textures.
            // HalfPixelOffset(1): realigns bloom / depth-of-field at upscaled resolutions.
            // NativeScaling(1): corrects artificially bright halos at non-native resolutions.
            // AutoFlush(2): clears GS buffer between passes, fixes shadow/lighting artifacts.
            // GPUPaletteConversion: offloads 8-bit palette work to GPU, reduces CPU spikes.
            s_settings_interface->SetIntValue("EmuCore/GS", "VsyncQueueSize", 8);
            s_settings_interface->SetBoolValue("EmuCore/GS", "HWMipmap", true);
            s_settings_interface->SetIntValue("EmuCore/GS", "HalfPixelOffset", 1); // Normal
            s_settings_interface->SetIntValue("EmuCore/GS", "NativeScaling", 1);   // Normal
            s_settings_interface->SetIntValue("EmuCore/GS", "UserHacks_AutoFlush", 2);
            s_settings_interface->SetBoolValue("EmuCore/GS", "GPUPaletteConversion", true);

            // Audio
            s_settings_interface->SetStringValue("SPU2/Output", "Backend", "SDL");

            // Speedhacks
            // MTVU off: ARM64 JIT race condition risk > throughput gain on mobile.
...
// Load gamepad button mapping from INI
    ...
    // One-time migration: apply new defaults to existing installs where keys are absent.
    auto migrate_str  = [&](const char* s, const char* k, const char* v) {
        if (!s_settings_interface->ContainsValue(s, k)) s_settings_interface->SetStringValue(s, k, v);
    };
    auto migrate_bool = [&](const char* s, const char* k, bool v) {
        if (!s_settings_interface->ContainsValue(s, k)) s_settings_interface->SetBoolValue(s, k, v);
    };
    auto migrate_int  = [&](const char* s, const char* k, int v) {
        if (!s_settings_interface->ContainsValue(s, k)) s_settings_interface->SetIntValue(s, k, v);
    };

    migrate_str ("SPU2/Output",            "Backend",              "SDL");
    migrate_bool("EmuCore/CPU",            "ExtraMemory",          false);

    // Clamping (VU Extra, EE Full)
    migrate_int ("EmuCore/CPU/Recompiler", "VU0ClampMode",         2);
    migrate_int ("EmuCore/CPU/Recompiler", "VU1ClampMode",         2);
    migrate_int ("EmuCore/CPU/Recompiler", "FPU.ClampMode",        3);

    // Gamefixes
    migrate_bool("EmuCore/Gamefixes",      "XgKickHack",           true);
    migrate_bool("EmuCore/Gamefixes",      "VuAddSubHack",         true);
    migrate_bool("EmuCore/Gamefixes",      "EETimingHack",         true);

    // GS quality / post-processing
    migrate_bool("EmuCore/GS",             "HWMipmap",             true);
    migrate_int ("EmuCore/GS",             "HalfPixelOffset",      1);
    migrate_int ("EmuCore/GS",             "NativeScaling",        1);
    migrate_int ("EmuCore/GS",             "UserHacks_AutoFlush",  2);
    migrate_bool("EmuCore/GS",             "GPUPaletteConversion", true);
